...
export const TextInput = forwardRef<HTMLInputElement, TextInputProps>(
  (
    ...
    const { extendRequiredFieldsLabelsWith, extendOptionalFieldsLabelsWith } =
      useDSFRConfig();
    const [inputState, setInputState] = useState("");
    const inputId = id || useId();
    const inputRef = useRef<HTMLInputElement>(null);
    const isInputStateControlled =
      message !== undefined || messageType !== undefined;

    const inputClass = cn(
      "fr-input",
      {
        "fr-input--error": isInputStateControlled
          ? messageType === "error"
          : inputState === "error",
        "fr-input--valid": isInputStateControlled
          ? messageType === "valid"
          : inputState === "valid",
      },
      css["fr-input"]
    ...
    const inputGroupClass = cn(
      "fr-input-group",
      {
        "fr-input-group--error": isInputStateControlled
          ? messageType === "error"
          : inputState === "error",
        "fr-input-group--valid": isInputStateControlled
          ? messageType === "valid"
          : inputState === "valid",
      ...
      <div className={inputGroupClass}>
        ...
        >
          <input
            ...
            aria-describedby={messageType ? `${inputId}-message` : undefined}
            {...props}
          />
        </div>
        {messageType && (
          <p className={`fr-${messageType}-text`} id={`${inputId}-message`}>
            {message}
          </p>
        )}
      </div>
