...
export const SegmentedControl: React.FC<SegmentedControlProps> = ({
  ...
  return (
    <fieldset className={_classes} id={id} {...attributes} {...props}>
      {label && (
        <legend
          className={classNames("fr-segmented__legend", {
            "fr-segmented__legend--inline": legendInline,
          })}
        >
          {label}
        </legend>
      ...
      {getChildrenOfType(children, SegmentedElement).map((child, index) => {
        if (React.isValidElement(child)) {
          return (
            <div
              className={classNames("fr-segmented__elements")}
              key={`segmented-element-${index}`}
            >
              {React.cloneElement(
                child as React.ReactElement<SegmentedElementProps>,
                {
                  name,
                  id: `${id}-${index}`,
                  onChange: () => {
